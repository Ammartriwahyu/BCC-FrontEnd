export const books = [
  {
    id: "kinki",
    title: "Tentang Suatu Wilayah di Daerah Kinki",
    author: "Sesuji Opi Ruhlinga",
    image: "/assets/images/Books/kinki.png",
    status: "ingin",
    description:
      "TEMAN SAYA MENGHILANG.Jika Anda memiliki informasi, mohon hubungi saya.Teman saya adalah editor baru yang sempat memegang proyek untuk edisi khusus sebuah majalah okultisme. Karena tugas tersebut, ia mengumpulkan dokumen dan materi dari berbagai sumber hingga akhirnya menyadari keanehan yang tersembunyi di suatu tempat di wilayah Kinki",
  },
  {
    id: "ikan",
    title: "Semua Ikan di Langit",
    author: "Ziggy Zezsyazeoviennazabrizkie",
    image: "/assets/images/Books/semuaIkandiLangit.png",
    status: "ingin",
    description:
      "Pekerjaan saya memang kedengaran membosankan— mengelilingi tempat yang itu-itu saja, diisi kaki-kaki berkeringat dan orang-orang berisik, diusik cicak-cicak kurang ajar, mendengar lagu aneh tentang tahu berbentuk bulat dan digoreng tanpa persiapan sebelumnya—tapi saya menggemarinya. ",
  },
  {
    id: "sunrise",
    title: "Sunrise On The Reaping",
    author: "Suzanne Collins",
    image: "/assets/images/Books/sunrise.png",
    status: "sedang",
    description:
      "When you've been set up to lose everything you love, what is there left to fight for?As the day dawns on the fiftieth annual Hunger Games, fear grips the districts of Panem. This year, in honor of the Quarter Quell, twice as many tributes will be taken from their homes.Back in District 12, Haymitch Abernathy is trying not to think too hard about his chances. All he cares about is making it through the day and being with the girl he loves.",
  },
  {
    id: "shark",
    title: "The Shark Caller",
    author: "Zillah Bethell",
    image: "/assets/images/Books/theShark.png",
    status: "ingin",
    description:
      "Karena sangat ingin menjadi pemanggil hiu untuk membalaskan dendam atas kematian orang tuanya, Blue Wing malah ditugaskan untuk berteman dengan pendatang baru yang menyebalkan, Maple. Awalnya mereka marah dan tidak cocok dengan pulau itu dan satu sama lain. Tetapi ketika ombak membawa janji harta karun, dapatkah mereka mengatasi perbedaan mereka dan menghadapi hiu paling mematikan di lautan?",
  },
  {
    id: "ronggeng",
    title: "Ronggeng Dukuh Paruk",
    author: "Ahmad Tohari",
    image: "/assets/images/Books/ronggeng.png",
    status: "ingin",
    description:
      "Gabungan 3 buku seri Dukuh Paruk: Ronggeng Dukuh Paruk, Lintang Kemukus Dini Hari & Jantera Bianglala.Semangat Dukuh Paruk kembali menggeliat sejak Srintil dinobatkan menjadi ronggeng baru, menggantikan ronggeng terakhir yang mati dua belas tahun yang lalu. Bagi pedukuhan yang kecil, miskin, terpencil dan bersahaja itu, ronggeng adalah perlambang. Tanpanya dukuh itu merasakah kehilangan jati diri.",
  },
  {
    id: "aroma",
    title: "Aroma Karsa",
    author: "Ahmad Tohari",
    image: "/assets/images/Books/aromaKarsa.png",
    status: "sedang",
    description:
      "Dari sebuah lontar kuno, Raras Prayagung mengetahui bahwa Puspa Karsa yang dikenalnya sebagai dongeng, ternyata tanaman sungguhan yang tersembunyi di tempat rahasia. Obsesi Raras untuk memburu dan menemukan Puspa Karsa, bunga sakti yang konon mampu mengendalikan kehendak dan hanya bisa diidentifikasi melalui aroma, mempertemukannya dengan Jati Wesi.",
  },
  {
    id: "walker",
    title: "The Song Walker",
    author: "Ahmad Tohari",
    image: "/assets/images/Books/songWalker.png",
    status: "dibaca",
    description:
      "Di tengah gurun gersang yang tak berbelas kasih, seorang gadis muda terbangun tanpa sedikit pun memori tentang siapa dirinya. Lalu, takdir mempertemukannya dengan Tarni, seorang gadis muda dari komunitas Penduduk Asli Australia yang tengah melakukan pencarian misterius. Bersama, mereka melintasi pedalaman Australia yang indah, memukau, tetapi juga penuh misteri.",
  },
];
